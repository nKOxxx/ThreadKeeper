...
- Focus on substantive content, not administrative details`;
    ...
    try {
      // Use Claude CLI to invoke Claude locally
      // The user's existing Claude Code auth is used automatically
      const prompt = `${systemPrompt}\n\n${userMessage}`;

      const result = execSync(`claude -s "${systemPrompt.replace(/"/g, '\\"')}" "${userMessage.replace(/"/g, '\\"')}"`, {
        encoding: 'utf-8',
      ...
      if (!jsonMatch) {
        console.error('No JSON found in Claude CLI response');
        return [];
      }
